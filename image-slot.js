...
(() => {
  ...
  class ImageSlot extends HTMLElement {
    ...
    _applyView() {
      ...
      // Focal point for cover: when a `position` is set and the user hasn't
      // manually reframed (no stored pan), anchor the crop toward that point
      // so an off-center subject (e.g. position="left") stays in frame.
      if (this.getAttribute('position') && !this._view.x && !this._view.y) {
        const pp = (kw) => kw === 'left' || kw === 'top' ? 0
          : kw === 'right' || kw === 'bottom' ? 1
          : kw === 'center' ? 0.5 : parseFloat(kw) / 100;
        const parts = this.getAttribute('position').trim().split(/\s+/);
        let fx = pp(parts[0]); let fy = pp(parts[1] != null ? parts[1] : 'center');
    ...
    _render() {
      ...
      // Alt text for SEO / accessibility — honor an explicit alt, else fall
      // back to the placeholder caption so filled images are never altless.
      const altText = this.getAttribute('alt') || this.getAttribute('placeholder') || '';
